...
import { KOREAN_LESSONS, type KoreanLesson } from "@/data/lessons/korean";
...
export default function KoreanPage() {
  ...
  const handleAnswer = useCallback((correct: boolean) => {
    ...
    if (q) addExp("korean", q.id, correct);
    setTimeout(() => {
      setFeedback(null);
    ...
    return (
      <div className="flex flex-col gap-5">
        <div className="text-center py-4">
          <h1 className="text-3xl font-black text-blue-700">📝 한글 학습</h1>
  ...
  if (phase === "result") {
    return (
      <div className="flex flex-col items-center gap-6 py-12">
        ...
        <h2 className="text-3xl font-black text-blue-700">Lesson {lesson.id} 완료!</h2>
        <p className="text-xl text-gray-600">
          {questions.length}문제 중 <span className="text-green-600 font-bold">{score}개</span> 맞았어요!
        </p>
        <div className="flex gap-3">
          <button onClick={() => { setCurrent(0); setScore(0); setPhase("quiz"); }}
            className="px-6 py-3 bg-blue-500 text-white rounded-2xl font-bold hover:bg-blue-600">
            다시 풀기
          </button>
          {lessonId < KOREAN_LESSONS.length && (
  ...
  return (
    <div className="flex flex-col gap-5">
      ...
      <div className="bg-white rounded-3xl p-6 shadow-lg">
        ...
        {(q.type === "trace" || q.type === "dictation") && (
          <div className="flex flex-col items-center gap-4">
            {q.image && <div className="text-6xl mb-2">{q.image}</div>}
            <p className="text-2xl font-bold text-gray-700">
              {q.type === "trace" ? `"${q.prompt}" 따라 써보자!` : "소리를 듣고 써보자!"}
            </p>
        ...
        {(q.type === "quiz" || q.type === "read") && q.choices && (
          <div>
            {q.type === "read" && (
              <div className="bg-amber-50 rounded-2xl p-4 mb-4">
                <p className="text-base text-gray-700 whitespace-pre-line leading-relaxed">{q.prompt}</p>
              </div>
            )}
            <QuizCard
              key={q.id}
              question={q.type === "read" ? "위 글에서 답을 찾아봐!" : q.prompt}
              image={q.type === "quiz" ? q.image : undefined}
              choices={q.choices}
